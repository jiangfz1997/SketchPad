...
public class CircleBrush implements DrawStrategy {
    private final Color color;
    private int x0, y0, x1, y1;
    private boolean dragging = false;

    public CircleBrush(Color color) {
        this.color = color;
    }
    ...
    public void onMouseReleased(MouseEvent e, List<Shape> shapeList) {
        if (dragging) {
            x1 = e.getX();
            y1 = e.getY();
            shapeList.add(new CircleShape(x0, y0, x1, y1, color));
            dragging = false;
        }
    }
    ...
    public void drawPreview(Graphics g) {
        ...
        g.drawOval(drawX, drawY, length, length);
    }
}
